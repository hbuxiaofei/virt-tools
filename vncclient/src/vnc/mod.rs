...
impl Image {
    fn new(format: PixelFormat, width: u16, height: u16) -> Result<Self, Error> {
        let image = match (format.depth, format.true_colour) {
            ...
            (8, false) => DynamicImage::ImageRgb16(ImageBuffer::<Rgb<u16>, Vec<u16>>::new(
                width.into(),
                height.into(),
            )),
            (d, t) => {
    ...
    fn set_colour_map(&mut self, first_colour: u16, colours: Vec<Colour>) -> Result<(), Error> {
        if colours.len() != 256 {
            return Err(Error::VncError(format!(
...
fn vnc_capture(target: &Target, opts: &Opts) -> Result<(), Error> {
    info!("Connecting to {:?}", target);
    let addr = match target {
        Target::Address(sock_addr) => sock_addr,
        Target::Url(_) => {
            return Err(Error::VncError(format!("Invalid VNC target: {}", target)));
        }
    };
